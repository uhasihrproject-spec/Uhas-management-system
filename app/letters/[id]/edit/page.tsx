...
export default async function EditLetterPage({
    ...
    return (
      <div className="p-8">
        <div className="rounded-3xl bg-white p-6 ring-1 ring-red-200/70">
          <p className="text-sm text-red-700">You must be logged in to edit letters.</p>
        </div>
      </div>
    );
  }
  ...
  if (!access.allowed || !canEdit) {
    return (
      <div className="p-8">
        <Link href={`/letters/${id}`} className="text-sm text-emerald-700 hover:underline">
          ← Back to Details
        </Link>
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-10 py-6 sm:py-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-3xl">
          <Link href={`/letters/${letter.id}`} className="text-sm text-emerald-700 hover:underline">
            ← Back to Letter Details
          </Link>

          <p className="mt-4 text-xs uppercase tracking-[0.25em] text-neutral-600">
            UHAS Procurement Directorate
          </p>

          <h1 className="mt-2 text-2xl font-semibold">Edit Letter Record</h1>

          <p className="mt-2 text-sm text-neutral-700">
            Update the information below and click <span className="font-medium">Save Changes</span>.
          </p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs text-emerald-700 ring-1 ring-emerald-100">
              Current file: <span className="font-medium">{letter.file_name}</span>
            </span>
          </div>
        </div>
