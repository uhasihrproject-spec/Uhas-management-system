...
function ActionPanel({ row, currentUserId, onPassed }: { row: PipelineRow; currentUserId: string; onPassed: (name: string | null, ref: string) => void }) {
  ...
  const handleConfirm = () => {
    ...
    startTransition(async () => {
      const res = isLast
        ...
        : await passToNext({ pipeline_id: row.pipeline_id, step_id: activeStep.id, remarks: remarks || undefined })
      if (!res.ok) {
        setError(res.error)
        return
      }
      ...
      {!open ? (
        ...
        <div className="max-w-lg rounded-2xl bg-neutral-50 p-4 ring-1 ring-neutral-200/70">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" onClick={handleConfirm} disabled={isPending} className="rounded-2xl bg-neutral-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-60">{isPending ? "Saving…" : "Confirm"}</button>
            <button type="button" onClick={() => { setOpen(false); setRemarks(""); setError(null) }} className="rounded-2xl border border-neutral-200 px-4 py-2 text-sm text-neutral-600">Cancel</button>
          </div>
...
function PipelineCard({ row, currentUserId, onPassed }: { row: PipelineRow; currentUserId: string; onPassed: (name: string | null, ref: string) => void }) {
      ...
      <div className="space-y-6">
        {mine.length > 0 && (
          <section>
            ...
            <div className="space-y-4">
              {mine.map(row => <PipelineCard key={row.pipeline_id} row={row} currentUserId={currentUserId} onPassed={handlePassed} />)}
            </div>
          </section>
        )}
        ...
        {others.length > 0 && (
          <section>
            ...
            <div className="space-y-4">
              {others.map(row => <PipelineCard key={row.pipeline_id} row={row} currentUserId={currentUserId} onPassed={handlePassed} />)}
            </div>
          </section>
        )}
